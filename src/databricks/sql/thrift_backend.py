...
class ThriftBackend:
    ...
    def open_session(self, session_configuration, catalog, schema):
        try:
            ...
            self._session_id_hex = (
                self.handle_to_hex_id(response.sessionHandle) if response.sessionHandle else None
            )
            return response
        except:
            self._transport.close()
    ...
    def _hive_schema_to_description(t_table_schema, session_id_hex=None):
        return [
            ThriftBackend._col_to_description(col, session_id_hex) for col in t_table_schema.columns
        ]
