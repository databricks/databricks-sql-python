...
class SEADatabricksClient(DatabricksClient):
        ...
        **kwargs
        ...
        if len(path_parts) >= 3:
            if path_parts[-2] in ["endpoints", "warehouses"]:
                ...
                logger.debug(f"Extracted warehouse ID: {self.warehouse_id} from path: {http_path}")
    ...
    def open_session(
        ...
        return SessionId.from_sea_session_id(session_id, {"warehouse_id": self.warehouse_id})
