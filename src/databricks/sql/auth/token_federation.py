...
from databricks.sql.auth.endpoint import get_databricks_oidc_url, get_oauth_endpoints, infer_cloud_from_host
...
class Token:
    ...
    def __init__(self, access_token: str, token_type: str, refresh_token: str = "", expiry: Optional[datetime] = None):
...
class DatabricksTokenFederationProvider(CredentialsProvider):
    ...
    def __init__(self, credentials_provider: CredentialsProvider, hostname: str, 
    ...
    def __call__(self, *args, **kwargs) -> HeaderFactory:
        ...
        def get_headers() -> Dict[str, str]:
            ...
            try:
                ...
                if (self.last_exchanged_token and self.last_external_token == access_token and 
                    self.last_exchanged_token.needs_refresh()):
        ...
        try:
            ...
            if self.idp_endpoints:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Failed to fetch OpenID configuration from {openid_config_url}: {response.status_code}")
...
class SimpleCredentialsProvider(CredentialsProvider):
    ...
    def __init__(self, token: str, token_type: str = "Bearer", auth_type_value: str = "token"):
        self.token = token
        self.token_type = token_type
...
def create_token_federation_provider(token: str, hostname: str, 
                                    identity_federation_client_id: Optional[str] = None,
                                    token_type: str = "Bearer") -> DatabricksTokenFederationProvider:
    """
    ...
    return DatabricksTokenFederationProvider(provider, hostname, identity_federation_client_id) 