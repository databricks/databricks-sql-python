...
class OAuthManager:
    ...
    def __get_authorization_code(self, client, auth_url, scope, state, challenge):
        ...
        if self.redirect_port is None:
            logger.error(f"Tried all the ports {self.port_range} for oauth redirect, but can't find free port")
            raise last_error
    ...
    def __send_auth_code_token_request(self, client, token_request_url, redirect_url, code, verifier):
    ...
    def get_tokens(self, hostname: str, scope=None):
        ...
        oauth_response = \
            self.__send_auth_code_token_request(client, token_request_url, redirect_url, code, verifier)
