...
class DependencyManager:
    ...
    def _extract_versions_from_specifier(self, spec_set_str):
        ...
        try:
            ...
            print(
                f"Warning: Could not parse constraint '{spec_set_str}': {e}",
                file=sys.stderr,
            )
            return None, None
    ...
    def _create_flexible_minimum_constraint(self, package_name, min_version):
        ...
        try:
            ...
            print(
                f"Warning: Could not create flexible constraint for {package_name}=={min_version}: {e}",
                file=sys.stderr,
            )
    ...
    def generate_requirements(self, version_type="min", include_optional=False):
        ...
        for name, constraint in self.dependencies.items():
            ...
            if version_type == "default":
                ...
                min_version, _ = self._extract_versions_from_specifier(
                    version_constraint
                )
                if min_version:
                    # Create flexible constraint that allows patch updates for compatibility
                    flexible_constraint = self._create_flexible_minimum_constraint(
                        name, min_version
                    )
    ...
    def write_requirements_file(
        self, filename, version_type="min", include_optional=False
    ):
        ...
        with open(filename, "w") as f:
            if version_type == "min":
                f.write(
                    f"# Minimum compatible dependency versions generated from pyproject.toml\n"
                )
                f.write(
                    f"# Uses flexible constraints to resolve compatibility conflicts:\n"
                )
                f.write(
                    f"# - Common packages (requests, urllib3, pandas): >=min,<next_major\n"
                )
                f.write(f"# - Other packages: >=min,<next_minor\n")
                f.write(
                    f"# - Includes transitive dependencies (e.g., numpy for pandas)\n"
                )
            else:
                f.write(
                    f"# {version_type.title()} dependency versions generated from pyproject.toml\n"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage dependency versions for testing"
    )
    parser.add_argument(
        "version_type", choices=["min", "default"], help="Type of versions to generate"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    requirements = manager.write_requirements_file(
        args.output, args.version_type, args.include_optional
    )
