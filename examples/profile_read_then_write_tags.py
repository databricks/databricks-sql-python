#!/usr/bin/env python3
"""
Profile information_schema.column_tags SELECT performance.

For each table, this script SELECTs existing column tags from
system.information_schema.column_tags. No ALTER/write operations.

Usage:
    python examples/profile_read_then_write_tags.py --threads 1 --iterations 1 --validate
    python examples/profile_read_then_write_tags.py --threads 8 --iterations 10
    python examples/profile_read_then_write_tags.py --threads 32 --iterations 10
"""
    ...
    with sql.connect(**conn_params()) as connection:
        with connection.cursor() as cursor:
            ...
            while True:
                ...
                # --- Step 1: Read column tags from information_schema ---
...
def generate_report(
    ...
    # --- Header ---
    w(f"# Information Schema Column Tags Profile: N={args.threads}, I={args.iterations}")
    w()
    w("## Configuration")
    ...
    w(f"- **Pattern**: SELECT from system.information_schema.column_tags per table")
    ...
    # --- All Operations with Statement IDs ---
    ...
    # --- Footer ---
    w("---")
    w(f"*Generated by profile_read_then_write_tags.py on {datetime.now().isoformat()}*")
...
def main():
    ...
    prefix = f"rw_n{args.threads}_i{args.iterations}"
