#!/usr/bin/env python3
"""
Profile SET COLUMN TAGS performance on Databricks.

Usage:
    # Quick validation (1 col x 1 tag x 1 thread x 1 iteration = 20 ALTERs)
    python examples/profile_column_tags.py --columns 1 --tags 1 --threads 1 --iterations 1 --validate

    # Single experiment
    python examples/profile_column_tags.py --columns 2 --tags 4 --threads 8 --iterations 10

    # Full sweep
    for c in 1 2 4; do
      for t in 1 2 4; do
        for n in 1 2 4 8 16; do
          python examples/profile_column_tags.py --columns $c --tags $t --threads $n --iterations 10
        done
      done
    done
"""
...
def build_alter_sql(table_fqn: str, column_name: str, num_tags: int) -> str:
    tags = ", ".join(f"'key{i}' = '{random_tag_value()}'" for i in range(1, num_tags + 1))
    return f"ALTER TABLE {table_fqn} ALTER COLUMN {column_name} SET TAGS ({tags})"
...
def setup_tables():
    """Create NUM_TABLES tables with MAX_COLUMNS STRING columns each."""
    print(f"Setting up {NUM_TABLES} tables with {MAX_COLUMNS} columns each...")
    with sql.connect(**conn_params()) as connection:
        with connection.cursor() as cursor:
            cursor.execute(f"USE CATALOG `{CATALOG}`")
            cursor.execute(f"USE SCHEMA `{SCHEMA}`")
            for t in range(1, NUM_TABLES + 1):
                cols = ", ".join(f"column{c} STRING" for c in range(1, MAX_COLUMNS + 1))
                ddl = f"CREATE TABLE IF NOT EXISTS table{t} ({cols})"
                print(f"  Creating table{t}...", end=" ", flush=True)
                cursor.execute(ddl)
                print("done")
...
def worker(
    ...
    """Worker thread: pulls tables from queue, ALTERs all columns, records metrics."""
    local_alter_results = []
    local_table_results = []
        ...
        with connection.cursor() as cursor:
            ...
            while True:
                ...
                for c in range(1, num_columns + 1):
                    ...
                    alter_sql = build_alter_sql(table_fqn, column_name, num_tags)
                ...
                table_latency_ms = (table_end - table_start) * 1000

                local_table_results.append(
                    {
                        "table": table_name,
                        "thread_id": thread_id,
                        "latency_ms": round(table_latency_ms, 2),
                        "num_alters": num_columns,
                        "num_errors": table_errors,
                        "alters_per_sec": round(num_columns / (table_latency_ms / 1000), 2)
                        if table_latency_ms > 0
                        else 0,
                    }
                )

    with results_lock:
        alter_results.extend(local_alter_results)
        table_results.extend(local_table_results)
...
def run_iteration(
    ...
    # Tag each result with iteration number
    for r in alter_results:
        r["iteration"] = iteration
    for r in table_results:
        r["iteration"] = iteration

    return alter_results, table_results, duration_s
...
def generate_report(
    ...
    total_alters = len(all_alter_results)
    total_duration = sum(iteration_durations)
    successful = [r for r in all_alter_results if r["success"]]
    failed = [r for r in all_alter_results if not r["success"]]
    success_latencies = [r["latency_ms"] for r in successful]
    all_latencies = [r["latency_ms"] for r in all_alter_results]

    # --- Header ---
    w(f"# Profile: C={args.columns}, T={args.tags}, N={args.threads}, I={args.iterations}")
    w()
    w("## Configuration")
    ...
    for i in range(1, args.iterations + 1):
        ...
        alters_in_iter = len([r for r in all_alter_results if r["iteration"] == i])
    ...
    # --- Per-Table Latency (all columns in one table) ---
    w("## Per-Table Latency — Time to Tag All Columns in One Table (ms)")
    w()
    w("| Table | Iteration | Thread | Latency (ms) | ALTERs/sec | Errors |")
    w("|-------|-----------|--------|--------------|------------|--------|")
    for r in sorted(all_table_results, key=lambda x: (x["iteration"], int(x["table"].replace("table", "")))):
        w(
            f"| {r['table']} | {r['iteration']} | {r['thread_id']} "
            f"| {r['latency_ms']:.2f} | {r['alters_per_sec']:.2f} | {r['num_errors']} |"
        )
    w()

    # --- Per-Table Aggregate Stats ---
    w("## Per-Table Aggregate Stats (ms)")
    w()
    table_latencies = [r["latency_ms"] for r in all_table_results]
    s = latency_stats(table_latencies)
    w("| Metric | Value |")
    w("|--------|-------|")
    for k, v in s.items():
        w(f"| {k} | {v:.2f} |")
    ...
    if not failed:
        ...
        for etype, records in sorted(error_groups.items(), key=lambda x: -len(x[1])):
            ...
            for r in records[:3]:
                w(f"- Table: {r['table']}, Column: {r['column']}, Iteration: {r['iteration']}")
    ...
    # Filter to ExecuteStatement only (excludes OpenSession, CloseSession, GetOperationStatus)
    ...
    # --- Footer ---
    w("---")
    w(f"*Generated by profile_column_tags.py on {datetime.now().isoformat()}*")
...
def main():
    ...
    prefix = f"c{args.columns}_t{args.tags}_n{args.threads}_i{args.iterations}"
    ...
    for i in range(1, args.iterations + 1):
        ...
        alter_results, table_results, duration = run_iteration(
            iteration=i,
        ...
        alters_count = len(alter_results)
        errors = len([r for r in alter_results if not r["success"]])
        rps = alters_count / duration if duration > 0 else 0
        print(f"done in {duration:.2f}s ({alters_count} ALTERs, {errors} errors, {rps:.1f} ALTERs/sec)")

    print()

    # Write raw data
    with open(data_path, "w") as f:
        for r in all_alter_results:
    ...
    # Generate report
    generate_report(args, all_alter_results, all_table_results, iteration_durations, profile_handler, report_path)
    ...
    success_lats = [r["latency_ms"] for r in all_alter_results if r["success"]]
    if success_lats:
        s = latency_stats(success_lats)
        total_dur = sum(iteration_durations)
        print()
        print("=== Summary ===")
        print(f"  ALTERs: {len(all_alter_results)} ({len(success_lats)} ok, {len(all_alter_results) - len(success_lats)} failed)")
        print(f"  Latency: p50={s['p50']:.1f}ms  p90={s['p90']:.1f}ms  p95={s['p95']:.1f}ms  p99={s['p99']:.1f}ms  max={s['max']:.1f}ms")
        print(f"  Throughput: {len(all_alter_results) / total_dur:.1f} ALTERs/sec")
