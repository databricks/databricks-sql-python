...
from databricks_sql_connector_core.sql import InterfaceError, DatabaseError, Error, NotSupportedError
...
class ClientTestSuite(unittest.TestCase):
    ...
    def test_close_uses_the_correct_session_id(self, mock_client_class):
        ...
        connection = databricks_sql_connector_core.sql.connect(**self.DUMMY_CONNECTION_ARGS)
        connection.close()
    ...
    def test_http_header_passthrough(self, mock_client_class):
        ...
        databricks_sql_connector_core.sql.connect(**self.DUMMY_CONNECTION_ARGS, http_headers=http_headers)
    ...
    def test_useragent_header(self, mock_client_class):
        ...
        user_agent_header = ("User-Agent", "{}/{}".format(databricks_sql_connector_core.sql.USER_AGENT_NAME,
                                                          databricks_sql_connector_core.sql.__version__))
        self.assertIn(user_agent_header, http_headers)

        databricks_sql_connector_core.sql.connect(**self.DUMMY_CONNECTION_ARGS, _user_agent_entry="foobar")
        user_agent_header_with_entry = ("User-Agent", "{}/{} ({})".format(
            databricks_sql_connector_core.sql.USER_AGENT_NAME, databricks_sql_connector_core.sql.__version__, "foobar"))
        http_headers = mock_client_class.call_args[0][3]
    ...
    def test_context_manager_closes_connection(self, mock_client_class):
        ...
        with databricks_sql_connector_core.sql.connect(**self.DUMMY_CONNECTION_ARGS) as connection:
            pass
    ...
    def test_max_number_of_retries_passthrough(self, mock_client_class):
        databricks_sql_connector_core.sql.connect(_retry_stop_after_attempts_count=54, **self.DUMMY_CONNECTION_ARGS)
    ...
    def test_socket_timeout_passthrough(self, mock_client_class):
        databricks_sql_connector_core.sql.connect(_socket_timeout=234, **self.DUMMY_CONNECTION_ARGS)
    ...
    def test_initial_namespace_passthrough(self, mock_client_class):
        ...
        databricks_sql_connector_core.sql.connect(**self.DUMMY_CONNECTION_ARGS, catalog=mock_cat, schema=mock_schem)
