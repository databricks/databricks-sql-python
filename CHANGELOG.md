...
# Unreleased
- Kernel backend (`use_kernel=True`): `session_configuration` is forwarded to the kernel verbatim (values stringified). Unlike the Thrift backend — which pins `spark.thriftserver.arrowBasedRowSet.timestampAsString` to `false` and rejects any non-false override — the kernel/SEA path neither injects nor validates that flag, since it is a Thrift-only `ExecuteStatement` conf with no meaning on the SEA session path (the kernel handles types natively via Arrow / complex-types).
