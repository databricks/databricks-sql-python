...
- Add `oauth_token_cache_enabled` for kernel U2M token caching.
- Forward kernel telemetry, socket timeout, OAuth, identity-federation, and row-limit settings.
- Preserve empty metadata filters and fix async statement telemetry.
- Lazy-load Apache Thrift for kernel and SEA connections.
- Deprecate SEA in favor of the kernel backend.
